...
class Home extends BaseController
{
	...
	{
		$data = [
			'title_meta' => view('partials/title-meta', ['title' => 'MDR']),
			'page_title' => view('partials/page-title', ['title' => 'Document', 'pagetitle' => 'MDR'])
		];
	...
	public function get_datatable() {
        $list = $this->project_model->getDatatables();
        $data = array();
        $itung = 0;

        foreach ($list as $baris) {
            if($baris->status == "Delayed"){
                $status = '<div class="badge badge-soft-danger font-size-12">'.$baris->status.'</div>';
            }else{
                $status = '<div class="badge badge-soft-success font-size-12">'.$baris->status.'</div>';
            }
            
            $aksi = '
                <span id="tooltip-container9">
                    <button type="button" class="btn btn-sm btn-danger btn-hapus" title="Hapus Data" data-id="'.$baris->id.'" data-object="Project/delete">
            ...
            $itung++;
            $row = array();
            $row[] = '<a href="project-dashboard" class="text-info fw-bold">'.$baris->contract_no.'</a>';
            $row[] = tgl_indo($baris->award_date);
            $row[] = rupiah($baris->value);
            $row[] = $baris->plan.'%';
            $row[] = $baris->progress.'%';
            $row[] = $baris->progress  -  $baris->plan .'%';
            $row[] = $aksi;
            $data[] = $row;
        }
        ...
        $output = array(
            ...
            "recordsFiltered" => $this->project_model->countFiltered(),
            "data" => $data,
        );
